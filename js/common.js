function inputFilter(obj,str)
{
	if(obj.value==str){obj.value='';obj.style.color='#c2c2c2';}
	obj.onblur = function(){if(this.value==''){this.value=str;}}
	$(".input_1 input").keydown(function(){
		$(this).css("color","red");
		}).focusout(function(){
...
function followState(type,target,uid){
	...
	if(type=='havefollow'){
		...
		html = '<a class="" style="font-size:14px;font-weight:bold;color:red;"href="javascript:void(0);" onclick="'+target+'(\'dofollow\',\''+target+'\',\''+uid+'\')">+加关注</a>';	}
	return html;
}
...
ui = window.ui ||{
	...
	/*	if (1 != $talkPop.data('type')) {
		...
		if(!this.emotdata){
			/**
			 * 加载表情
			 * header.html --> var _PUBLIC_ = '${base}/resbase/sites/whaty/public';
			 */
			$.ajax({
				   type: "GET",
				   url:  _PUBLIC_ + "/js/emotions.json",
	...
	countNew:function(){
		...
		$.getJSON( url ,function(txt){
			if(txt.message!="0"){
				$("#count_message").html('<div class="ms3" id="count_message">'+txt.message+'</div>');
			}else{
				$("#count_message").html('');
			}
			/*
			...
			$("#count_classmates").html('('+txt.classmates+')');
			$("#count_myTrends").html('('+txt.myTrends+')');
			$("#count_allTrends").html('('+txt.allTrends+')');
			*/
	...
	getarea:function(prefix,init_style,init_p,init_c){
		...
		document.write(html);
		// _PUBLIC_+'/js/area.js'
		$.getJSON( _PUBLIC_ + "/js/area.json", function(json){
			json = json.provinces;
